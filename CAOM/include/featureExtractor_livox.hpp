/////////////////////////////////////////////
// Created by joe on 2020/11/14.
// Modified from livox Inc.
//
// extract feature points from /livox/lidar
////////////////////////////////////////////////////////////////

#ifndef STRUCTURAL_MAPPING_FEATUREEXTRACTOR_LIVOX_HPP
#define STRUCTURAL_MAPPING_FEATUREEXTRACTOR_LIVOX_HPP

#include "tools.h"

class FeatureExtractorLivox{

public:
    FeatureExtractorLivox(){}

    static bool plane_judge(const std::vector<PointType>& point_list, const int plane_threshold){
        ...
        return (saes.eigenvalues()[2] > plane_threshold * saes.eigenvalues()[1]);
    }

    static void extract(const pcl::PointCloud<PointType>& laserCloudIn,
                        pcl::PointCloud<PointType>::Ptr cornerCloudout,
                        pcl::PointCloud<PointType>::Ptr surfCloudout){
        ...
        for (int i = 0; i < cloudSize; i++) {
            ...
            int scanID = std::floor(theta / 9);  // TODO why 9?
        ...
        int* CloudFeatureFlag = new int[cloudSize]{0};
//        for (int i = 0; i < cloudSize; i++)
...
//                std::vector<PointType> left_list;  // Unused
//                for(int j = -4; j < 0; j++)
...
//                    right_list.push_back(laserCloud->points[i+j]);
            ...
            if(fabs(diff_right[0] - diff_left[0])>0.1){
                if(diff_right[0] > diff_left[0]){
                    ...
                    if(left_is_plane && (max_dis < 2*min_dis) && left_surf_dis < 0.05 * depth  && cc < 0.8){//
                        ...
                        else if(depth_right == 0) CloudFeatureFlag[i] = 100;
                    }
                }
                else{
        ...
        for(int i = 0; i < cloudSize; i++){
            ...
            if(CloudFeatureFlag[i] == 1){
                ...
                surfCloudout->points.push_back(laserCloud->points[i]);
                continue;
            }
        ...
        delete CloudFeatureFlag;

        std::cout<<"[ LivoxFea ] ALL point: "<< cloudSize << " outliers: " << debugnum1 << std::endl
                 <<" break points: "<< debugnum2<<" break feature: "<< debugnum3 << std::endl
                 <<" normal points: "<< debugnum4<<" surf-surf feature: " << debugnum5 << std::endl;

        *cornerCloudout = cornerPointsSharp;
    }

};

#endif //STRUCTURAL_MAPPING_FEATUREEXTRACTOR_LIVOX_HPP
